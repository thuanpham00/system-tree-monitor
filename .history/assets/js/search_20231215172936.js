...
headerInput.addEventListener("keydown", async function (e) {
  const searchItem = e.target.value;
  console.log(searchItem);
  const path =
    searchItem !== ""
      ? `${url1}?nameItem_like=${searchItem}` && console.log("tìm thấy dữ liệu")
      : url1 && console.log("không tìm thấy dữ liệu");
  await fetchSearch1(path);
});
