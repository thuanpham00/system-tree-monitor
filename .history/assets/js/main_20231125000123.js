window.addEventListener("load", function (e) {
    ...
    [...serviceLinkItem].forEach((item) =>
        item.addEventListener("click", function (e) {
            if (e.target.dataset.tree == "0") {
                const item = listSickTree[e.target.dataset.tree]
                serviceHeading.textContent = item.name
            } else if (e.target.dataset.tree == "1") {
                console.log("hehee");
            }
        })
    );
    ...
    //     let sickNumber = listSickTree[numberSick]; // lấy ra phần tử
    //     document.querySelector(".service__heading.title").textContent = sickNumber.name;
    //     document.querySelector(".sick__desc.desc-1").textContent = sickNumber.text1;
    //     document.querySelector(".sick__desc.desc-1.desc-2").textContent = sickNumber.text2;
    //     document.querySelector(".sick__desc.desc-1.desc-3").textContent = sickNumber.text3;
    // }

    // document
    //     .querySelector(".service__link")
    //     .addEventListener("click", function (event) {
    //         event.preventDefault(); // Ngăn chặn hành động mặc định của sự kiện click
    //         window.location.href = "template.html"; // Chuyển hướng trang đến trang mới
    //     });
});
