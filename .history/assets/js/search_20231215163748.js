...
const pathNews = http.get("/listTinTuc");
const fetchDataSearch_New = async (link = pathNews) => {
  await pathNews.then((res) => {
    const data = res.data;
...
const endPointSearch_1 = `https://dm92v6-8080.csb.app/listTinTuc`
...
headerInput.addEventListener(
  ...
  debounceFn((e) => {
    ...
    let path = pathNews;
    if (e.target.value !== "") {
      path = `${http.get(/listTinTuc&nameItem_like)}=${e.target.value}`;
    }
